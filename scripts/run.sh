...
BANNER
  ...
  if [[ -n "$phala_cloud_key" ]]; then
    if [[ ${#phala_cloud_key} -ge 12 ]]; then
      masked="${phala_cloud_key:0:4}…${phala_cloud_key: -4}"
    else
      masked="(set; ${#phala_cloud_key} chars)"
    fi
    echo "  ✓ Phala Cloud API key recorded: ${masked}"
  else
    echo "  ⏭ Skipped (you'll need to run 'phala login' once before Day 2)"
  fi
