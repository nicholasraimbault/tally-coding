...
- [x] Sub-project B4 implemented: Tally narrator (Red Pill LLM + spend guard + 5-min sweeper + event-driven on task state transitions); escalation routing (task channel → long-term channel via `get_workspace_escalation_channel_id` + `route_escalation_to_long_term_channel` with quick_reply_options + queue_position); push notification dispatch with JSON payload (`emit_escalation_push`); Flutter `EscalationNotifier` (Android NotificationAction per option + iOS APNs `tally_escalation` category); action taps wired to `postMessage` for replies + `openChannel` for "Open". 322 orchestrator tests + 277 Flutter tests passing.
...
- [x] Sub-project B5 implemented: SidebarShell (WorkspaceRow + SidebarChannelsList + SidebarMiniDash) replaces ServerRail + _ChannelList + _MembersPanel in DiscordShellScreen's wide layout at >=1100px breakpoint. SidebarMiniDash has both ambient + escalation takeover (no drag handle on desktop; stacked vertical quick replies). 11/12 tasks; Task 9 (MiniDashController integration) skipped because that controller name was planned but B3 shipped BottomSheetController with a different API. Sidebar uses stub data from _tasks until a unified controller surfaces openCount/doneToday/runningTasks/narratorText. 277 Flutter tests passing.
- [x] Sub-project B fully shipped: B1 + B2 + B3a + B3b + B4 + B5 all landed on `feat/sprint-52-admin-gaps-audit-polish`. 60+ commits since the spec write; 277 Flutter tests + 322 orchestrator tests; analyzer clean (22 pre-existing info-level issues, no functional issues).
- [ ] Sub-project A (first-task flow) plan written.
- [ ] Sub-project C (wrap-up flow) plan written.
- [ ] Sub-project D (Gen-UI catalog) plan written (post-MVP).
- [ ] Sub-project B6 / pre-D (Tally coordination monitor for delegated parallel agent work) plan written. Context: project_tally_coordination_monitor memory. Identified when Agent tool's isolation:"worktree" silently failed during B4+B5 parallel execution — both agents landed commits on the same branch, B4 observed B5's WIP-broken test state mid-run. Tally should monitor parallel-delegated agent work, detect shared-branch commits + file overlap + cross-agent test regressions, and escalate via the mini dash + push notif surface that B3a/B3b/B4 just shipped.
